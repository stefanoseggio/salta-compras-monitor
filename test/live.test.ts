...
// host with no uptime guarantee). Kept in its own file, separate from
// fetchListing.test.ts's `vi.mock('impit', ...)`, because Vitest module
// mocks are file-scoped - if this describe block lived in the same file as
// that mock, these "live" tests would silently hit the mocked impit.fetch()
// (which resolves to undefined with no implementation set) instead of the
// real network.
